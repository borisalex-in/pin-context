...
export class GitContextService {
    ...
    async getCurrentWorkspaceGitContexts(): Promise<GitContextInfo[]> {
        ...
        const results = await Promise.allSettled(folders.map(folder => this.getGitInfoForFolder(folder.uri.fsPath)));
        return results
            .filter((result): result is PromiseFulfilledResult<GitContextInfo> => result.status === 'fulfilled')
