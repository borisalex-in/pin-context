...
import { ContextSnapshot, ContextTimelineEntry, PinContext } from './contextTypes';
...
export class ContextStore implements vscode.Disposable {
    ...
    getAllContexts(): PinContext[] {
        return [...this.contexts.values()].sort((a, b) => b.updatedAt - a.updatedAt);
    }

    getTimelineBuckets(): { today: ContextTimelineEntry[]; yesterday: ContextTimelineEntry[]; older: ContextTimelineEntry[] } {
        const now = new Date();
    ...
    async deleteContext(contextId: string): Promise<boolean> {
        ...
        this.timeline = this.timeline.filter(entry => entry.contextId !== contextId);
        await this.persist();
        this.onDidChangeEmitter.fire();
        return true;
    }

    async switchContext(contextId: string, action: 'switch' | 'restore' = 'switch'): Promise<boolean> {
        ...
        try {
            const targetUris = target.pinnedUris.map(value => vscode.Uri.parse(value));
            ...
            const toUnpin = current.filter(uri => !targetKeys.has(toUriKey(uri)));
            const toPin = targetUris.filter(uri => !currentKeys.has(toUriKey(uri)));

            if (toUnpin.length > 0) {
                await this.pinStore.unpinUris(toUnpin, { showProgress: toUnpin.length > 15, title: `Switching to ${target.name}` });
            }
            if (toPin.length > 0) {
                await this.pinStore.pinUris(toPin, { showProgress: toPin.length > 15, title: `Switching to ${target.name}` });
            }
        } finally {
    ...
    async refreshGitContexts(): Promise<void> {
        if (!this.getConfig<boolean>('contexts.autoGitContexts', false)) {
            return;
        }
        ...
        for (const git of gitContexts) {
            ...
            const name = `${git.branch} (${git.changed}/${git.staged})`;
            const pinnedUris = existing?.pinnedUris ?? [];
    ...
    private pushTimeline(contextId: string, contextName: string, action: ContextTimelineEntry['action']): void {
        if (!this.getConfig<boolean>('contexts.timelineEnabled', true)) {
            return;
        }
        ...
        const maxEntries = this.getConfig<number>('contexts.maxTimelineEntries', 100);
        this.timeline = this.timeline.slice(0, maxEntries);
    }

    private load(): void {
        const snapshot = this.getMemento().get<ContextSnapshot | undefined>(CONTEXTS_STATE_KEY);
        if (!snapshot) {
            return;
        }
        this.contexts.clear();
        for (const contextItem of snapshot.contexts) {
            this.contexts.set(contextItem.id, contextItem);
        }
    ...
    private async persist(): Promise<void> {
        ...
        await this.getMemento().update(CONTEXTS_STATE_KEY, snapshot);
    }

    private getMemento(): vscode.Memento {
        const scope = this.getConfig<'globalState' | 'workspaceState'>('contexts.persistenceScope', 'globalState');
        return scope === 'workspaceState' ? this.context.workspaceState : this.context.globalState;
    }
    ...
    private async syncActiveContextFromPins(): Promise<void> {
        ...
        const next = this.pinStore.getPinnedUris().map(uri => uri.toString());
        const previous = active.pinnedUris;
