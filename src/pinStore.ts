...
export class PinStore implements vscode.Disposable {
    ...
    async pinEditorsByPattern(pattern: string): Promise<BatchOutcome> {
        const matchingFiles = await vscode.workspace.findFiles(pattern, undefined, this.getFindFilesLimit());
        ...
        const targets = this.getOpenTabs()
            .filter(tab => {
                ...
                return matchingKeys.has(toUriKey(uri)) || minimatch(uri.path, pattern, { nocase: true, dot: true });
            })
            .map(tab => getPrimaryTabUri(tab))
            .filter((uri): uri is vscode.Uri => Boolean(uri));
        ...
        return this.enqueueBatch(async () => {
            ...
            const execution = async (
                ...
                for (const batch of batches) {
                    const results = await Promise.allSettled(batch.map(uri => this.applyPinStateForUri(uri, shouldPin)));
