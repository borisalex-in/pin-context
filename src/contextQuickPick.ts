...
import { ContextQuickPickItem } from './contextTypes';
...
async function pickContext(contextStore: ContextStore, title: string): Promise<ContextQuickPickItem | undefined> {
    const activeId = contextStore.getActiveContext()?.id;
    const manualContexts = contextStore.getAllContexts().filter(context => context.source === 'manual');
